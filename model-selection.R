...
> AIC(fit47, fit48, fit49, fit50, fit51, fit52, fit53, fit54, fit55, fit56)
...
Call:
glm(formula = Ch ~ Dir + Edi + Dan + Gdr + Gd + PGA + DGA + Romance + 
...
Number of Fisher Scoring iterations: 6
 ...
 > step_bic <- step(full_model, direction = "both", k = log(nrow(oscars)))
> summary(step_bic)
Call:
glm(formula = Ch ~ Dir + Edi + PGA, family = binomial, data = oscars)
...
Model 1: Ch ~ Dir + Edi + Dan + Gdr + Gd + PGA + DGA + Romance + SciFi + 
    Days + PG + PG13 + R + NSFC + WR
Model 2: Ch ~ Nom + Dir + Aml + Afl + Ams + Afs + Scr + Cin + Art + Cos + 
...
Model 1: Ch ~ Dir + Edi + PGA
Model 2: Ch ~ Dir + Edi + Dan + Gdr + Gd + PGA + DGA + Romance + SciFi + 
    Days + PG + PG13 + R + NSFC + WR
  Resid. Df Resid. Dev Df Deviance  Pr(>Chi)    
1       600     324.47                          
2       588     281.92 12    42.55 2.691e-05 ***> anova(step_aic, full_model, test = "LRT")
Analysis of Deviance Table
